...
<div class="prereq">
<p><strong>Prerequisites:</strong> the whole course, and especially the previous four lessons — numerical stability (<a href="#/lessons/module-12/lesson-01">35</a>), automatic differentiation (<a href="#/lessons/module-12/lesson-02">36</a>), the end-to-end model (<a href="#/lessons/module-12/lesson-03">37</a>), and fine-tuning (<a href="#/lessons/module-12/lesson-04">38</a>). Backprop mechanics from <a href="#/lessons/module-08/lesson-03">25 · Backpropagation and autograd</a> and optimizers from <a href="#/lessons/module-09/lesson-02">27 · AdamW</a> and <a href="#/lessons/module-09/lesson-03">28 · Muon</a> are used constantly.</p>
...
**What tensors exist afterward:** all the activations from [lesson 37](lessons/module-12/lesson-03.md) — the embedded $x$ of shape $(B,T,C)$, every block's intermediates, `logits` of shape $(B,T,V)$, and the scalar `loss` — plus, crucially, a **computational graph** connecting them, recorded as the code ran ([lesson 36](lessons/module-12/lesson-02.md)).
...
**Where derivatives come in:** none are *computed* yet — but the graph is *built for* them. Each operation on a tracked tensor attaches a `grad_fn` (its vector-Jacobian-product function, [lesson 36](lessons/module-12/lesson-02.md)) and records which tensors fed it. PyTorch also keeps the activations these VJPs will need during the backward pass. This is the "record" phase of reverse-mode AD.
...
**What tensors exist afterward:** every parameter's `.grad` is now filled — one gradient tensor per parameter, each the same shape as its parameter ([lesson 37](lessons/module-12/lesson-03.md)). The graph and the activations it held are, by default, **freed**.

**What math happens — the heart of training.** Reverse-mode automatic differentiation ([lesson 36](lessons/module-12/lesson-02.md)):

1. Seed $\dfrac{\partial L}{\partial L} = 1$ at the loss node.
2. Walk the recorded graph in reverse topological order — each node processed only after everything it fed has been processed ([lesson 25](lessons/module-08/lesson-03.md)).
...
<div class="callout key"><p><code>loss.backward()</code> is the entire calculus half of the course executed at once: the chain rule (module 6–7), applied as vector-Jacobian products (module 7), over the graph recorded in the forward pass (module 5, lesson 36), depositing $\partial L/\partial\theta$ into every parameter's <code>.grad</code> (module 8). Nothing here is magic — it is the by-hand backward pass of <a href="#/lessons/module-08/lesson-01">lesson 23</a>, automated.</p></div>
...
**What math happens:** for each parameter the optimizer combines the gradient with its state to produce an update. For AdamW ([lesson 27](lessons/module-09/lesson-02.md)), per parameter:
...
where $g$ is the parameter's `.grad`, $\eta$ the learning rate, $\lambda$ the weight decay, and $\varepsilon$ the stability term from [lesson 35](lessons/module-12/lesson-01.md). For Muon ([lesson 28](lessons/module-09/lesson-03.md)) the 2-D hidden matrices are updated by their orthogonalized gradient instead.
...
**Under the hood:** `step()` runs inside `torch.no_grad()` — the update $\theta \leftarrow \theta - \ldots$ must not be recorded as a graph operation, or it would corrupt the next backward pass ([lesson 25](lessons/module-08/lesson-03.md)). After `step()`, the parameters are new; the loop returns to `zero_grad()` and repeats with the next batch.
...
**Line 8 — gradient clipping.** `clip_grad_norm_` measures the global gradient norm across all parameters and, if it exceeds `max_norm=1.0`, rescales the whole gradient down to that norm — capping length, preserving direction ([lesson 35](lessons/module-12/lesson-01.md)). This is the standard guard against gradient explosion, placed after all accumulation and before the step.

**Line 9 — optimizer step.** Consume `.grad` and the optimizer state to update parameters (section 5). In a two-optimizer setup ([lesson 28](lessons/module-09/lesson-03.md)) you would call `.step()` on both the AdamW and the Muon optimizer here.
...
<div class="callout pt"><p>The extra lines are all refinements of the core four. autocast is <a href="#/lessons/module-12/lesson-01">lesson 35</a>'s precision tradeoff; clipping is its explosion guard; the scheduler tunes $\eta$; accumulation exploits that <code>backward()</code> sums into <code>.grad</code> (<a href="#/lessons/module-08/lesson-03">lesson 25</a>) to average several micro-batches into one virtual big batch. Strip them away and <code>zero_grad → forward → backward → step</code> remains.</p></div>
...
- **What every tensor means.** Tokens are integer IDs; embeddings turn them into vectors; hidden states are $(B,T,C)$ activations; logits are $(B,T,V)$ scores; the loss is one scalar. Each is a **parameter, activation, gradient, or optimizer state** — the four kinds of quantity. *(modules 2–4, [lesson 37](lessons/module-12/lesson-03.md))*
- **What every dimension means.** $B$ batch, $T$ sequence, $C$ channels/$d_\text{model}$, $V$ vocabulary, $n_h$ heads, $d_h$ head dim with $C=n_h d_h$; reshapes and transposes just re-view the same numbers to expose the head axis. *(module 4, [lesson 32](lessons/module-11/lesson-01.md))*
...
- **Where derivatives are computed.** In `loss.backward()`, and nowhere else — one reverse-mode sweep of vector-Jacobian products over the graph the forward pass recorded. *(modules 6–7, [lesson 36](lessons/module-12/lesson-02.md))*
- **Where gradients are stored.** In each parameter's `.grad`, one tensor per parameter, same shape, *accumulated* (hence `zero_grad`). *(module 8, [lesson 25](lessons/module-08/lesson-03.md))*
- **How gradients flow backward.** From the loss, through the head and every block — including the multiply-by-one residual paths that keep them from vanishing — back to the embeddings, by the chain rule. *(modules 6–7, [lesson 33](lessons/module-11/lesson-02.md), [lesson 35](lessons/module-12/lesson-01.md))*
- **How optimizer state updates.** AdamW updates first/second moments $m,v$ per parameter; Muon updates a momentum buffer per matrix and orthogonalizes it. State persists across steps; gradients do not. *(module 9, [lesson 27](lessons/module-09/lesson-02.md), [lesson 28](lessons/module-09/lesson-03.md))*
- **How parameters change.** Once per optimizer step, in place, under `no_grad`: $\theta \leftarrow \theta - \eta(\ldots)$ — the only line that moves the weights. *(module 9, [lesson 26](lessons/module-09/lesson-01.md))*
...
The update $\theta \leftarrow \theta - \eta(\ldots)$ modifies parameters; it is not part of the function being differentiated. Recording it in the graph would corrupt the next backward pass and waste memory. `no_grad` tells autograd not to track it. *(See [lesson 25](lessons/module-08/lesson-03.md).)*
